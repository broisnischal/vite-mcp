...
export const consoleAdapterInputSchema = z.object({
    limit: z.number().int().positive().optional().default(100).describe('Maximum number of console messages to return'),
    type: z.enum(['log', 'info', 'warn', 'error', 'debug']).optional().describe('Filter console messages by type'),
});
